...
public class ExcelUtil {
    ...
    public static void setExcelFileSheet(String sheetName) {
        ...
        // Open the Excel file
        Log.allure("Platform.getCurrent().toString(): %s",Platform.getCurrent().toString());
        Log.allure("Path excel: %s",testDataExcelPath);
        Log.allure("currentDir: %s",currentDir);
        FileInputStream ExcelFile = new FileInputStream(testDataExcelPath + testDataExcelFileName);
        excelWBook = new XSSFWorkbook(ExcelFile);
        excelWSheet = excelWBook.getSheet(sheetName);
        Log.allure("File name: %s",excelWBook.getSheetName(0));
    }
    ...
    public static void setCellData(String value, int RowNum, int ColNum) {
        row = excelWSheet.getRow(RowNum);
        Log.allure("Row: %s",String.valueOf(row));
        cell = row.getCell(ColNum);
        Log.allure("Row: %s",String.valueOf(cell));
        if (cell == null) {
